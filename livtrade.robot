...
Завантажити документ
    ...
    ...    ${ARGUMENTS[2]} == ${TENDER}
    livtrade.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[2]}
...
Отримати інформацію із предмету
    ...
    ${return_value}=    Run Keyword And Return    livtrade.Отримати інформацію із тендера    ${username}    ${tender_uaid}    ${field_name}
    [Return]    ${return_value}
...
Перейти на сторінку тендера
    ...
    Run Keyword Unless  ${present}  livtrade.Пошук тендера по ідентифікатору  ${username}  ${tender_uaid}
...
Отримати інформацію про dgfDecisionDate
    ...
    ${return_value}=    livtrade_service.convert_date    ${date_value}
    [Return]    ${return_value}
